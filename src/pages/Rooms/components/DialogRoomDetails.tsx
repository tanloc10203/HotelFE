    ...
    >
      ...
      <DialogContent dividers>
        <Stack flexDirection={"row"} gap={1} flexWrap={"wrap"} mb={2}>
          {row?.roomType?.images?.map((image) => (
            <LazyLoading src={image.src} sxBox={{ width: 100, height: 100 }} />
          ))}
        </Stack>
        ...
        >
          ...
          <CardDashed
            label="Giá phòng theo giờ"
            data={`${fCurrency(row?.roomType?.prices?.price_hour! ?? 0)}`}
            keyData="name"
            colorChip={colors("default", theme).colorActive}
            backgroundChip={colors("default", theme).bgActive}
          />
        ...
        >
          <CardDashed
            label="Giá phòng qua app theo ngày"
            data={`${fCurrency(row?.roomType?.prices?.price_day_online! ?? 0)}`}
            keyData="name"
            colorChip={colors("red", theme).colorActive}
            backgroundChip={colors("red", theme).bgActive}
          />
          ...
          <CardDashed
            label="Giá phòng qua app theo giờ"
            data={`${fCurrency(row?.roomType?.prices?.price_hour_online! ?? 0)}`}
            keyData="name"
            colorChip={colors("red", theme).colorActive}
            backgroundChip={colors("red", theme).bgActive}
          />
