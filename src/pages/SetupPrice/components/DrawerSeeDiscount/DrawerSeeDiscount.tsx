...
const DrawerSeePriceList: FC<DrawerSeePriceListProps> = ({ open, data, onClose }) => {
  ...
  return (
    <Drawer
      ...
      PaperProps={{ sx: { borderRadius: `12px 0 0 12px`, width: "85%" } }}
      ...
      <DialogContent sx={{ width: "auto", ...SCROLLBAR_CUSTOM }} role="presentation">
        <Stack flexDirection={"row"} gap={2}>
          ...
          <TextField
            ...
            value={fDateDayjs(dayjs(data.start_time), "DD/MM/YYYY HH:mm")}
            disabled
            size="small"
            sx={{
              "& input": {
          ...
          <TextField
            ...
            value={fDateDayjs(dayjs(data.end_time), "DD/MM/YYYY HH:mm")}
            disabled
            size="small"
            sx={{
              "& input": {
        ...
        <Stack mt={4} gap={2}>
          <Typography fontWeight={700}>Chi tiết bảng giá phòng khuyến mãi</Typography>
