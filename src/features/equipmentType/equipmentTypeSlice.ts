...
const equipmentTypeSlice = createSlice({
  ...
  reducers: {
    ...
    getDataSuccess: (
      ...
      }: PayloadAction<SuccessResponseProp<IEquipmentType[], Pagination>>
    ) => {
      state.data = metadata;
    ...
    addDataStart: (state, _: PayloadAction<IEquipmentType & ResetFormType>) => {
    ...
    editDataStart: (state, _: PayloadAction<IEquipmentType & ResetFormType>) => {
      if (state.errors.addEdit) state.errors.addEdit = "";
      state.isLoading = "pending";
    },

    deleteDataStart: (state, _: PayloadAction<IEquipmentType>) => {
      state.isLoading = "pending";
    },
